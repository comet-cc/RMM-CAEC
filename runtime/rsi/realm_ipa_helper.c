				    ...
				    struct s2_walk_result *s2_walk)
{
	...
	if (s2tte_is_assigned_ram(s2_ctx, s2tte, wi.last_level)) {
		...
		if (s2tte_is_unassigned_destroyed(s2_ctx, s2tte) ||
		    s2tte_is_assigned_destroyed(s2_ctx, s2tte, wi.last_level)) {
			s2_walk->ripas_val = RIPAS_DESTROYED;
		} else if (s2tte_is_unassigned_ram(s2_ctx, s2tte)) {
			s2_walk->ripas_val = RIPAS_RAM;
		} else {
...
{
	const struct s2tt_context *m_ctx = &master_rd->s2_ctx;
    const struct s2tt_context *s_ctx = &slave_rd->s2_ctx;
    struct s2tt_walk wi_m, wi_s;
    unsigned long *s2tt_m, *s2tt_s;
	...
	if (!(is_2mb_aligned(master_ipa) && is_2mb_aligned(slave_ipa))){
 	...
 	granule_lock(m_ctx->g_rtt, GRANULE_STATE_RTT);
    s2tt_walk_lock_unlock(m_ctx, master_ipa, S2TT_PAGE_LEVEL, &wi_m);
    if (wi_m.last_level != S2TT_PAGE_LEVEL) {
        granule_unlock(wi_m.g_llt);
        return 0;
    }

	s2tt_m = buffer_granule_map(wi_m.g_llt, SLOT_RTT);
    assert(s2tt_m != NULL);
